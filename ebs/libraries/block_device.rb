module BlockDevice
  ...
  def self.assemble_raid(raid_device, options)
    ...
    unless exec_command("mdadm --assemble --verbose #{raid_device} #{options[:disks].join(' ')}")
      Chef::Log.info("As resuming existing RAID array #{raid_device} failed in the first try, the RAID array is temporarily deactivated and blocking kernel modules are unloaded")
      exec_command("mdadm --stop --verbose #{raid_device}")
      sleep 5
      exec_command("rmmod md_mod")
      sleep 5
